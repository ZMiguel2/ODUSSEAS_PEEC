...
convolve_now = False  # convolve now if you want to add a new resolution dataset
resolution = 115000  # set the resolution that you want to convolve the HARPS stars.

resonumber = str(resolution)

c = 299792

if convolve_now == True:
    DU_harps = c / 115000
    R_new = resolution
    DU_new = c / R_new
...
def Load_Data_Pairs(data1):
    ...
    for file in listdir:
        ...
        if file.endswith(".dat"):
            ...
            our_datanames.append(temp_name.split("/")[4].strip(".dat"))
            # append data
...
if convolve_now == True:
    filepaths = np.loadtxt("Files/RefHARPSfilelist.dat", dtype=str)
    for item in filepaths:
        convolve_data(item, R_conv, resonumber)

    # below I create the convolved HARPSfilelist
    name_of_input = "Files/RefHARPSfilelist.dat"
    ...
    name_of_output = "Files/RefHARPSfilelist.dat"

filepaths = np.loadtxt(name_of_output, dtype=str)

wavelength_range = np.loadtxt("Files/lines.rdb", skiprows=2)
dw = 0.4
plot = False

Interm_results_path = "Results/Interm_Results" + "/"
directory_name = Interm_results_path + "res" + resonumber + "RefHARPS_EWmyresults"
...
myData = "./" + Interm_results_path + "res" + resonumber + "RefHARPS_EWmyresults"
...
np.savetxt("Files/centrallines.dat", wcentral)
lines = np.loadtxt("Files/centrallines.dat")
...
np.savetxt("./" + Interm_results_path + 
    "res" + resonumber + "RefHARPS_myEW.dat", table, header=headers, delimiter=","
)

# transpose
table = np.loadtxt("./" + Interm_results_path + 
...
df = pd.read_csv("Files/Refparameters.dat", sep=" ", header=0)

# df.loc[:, "starname"] = df.Star.str.split("_").str[0].str.strip()


df2 = pd.read_csv("./"+ Interm_results_path + "res" + resonumber + "RefHARPS_transposed.csv", sep=",")
...
df3 = pd.merge(df2, df, on="starname", how="inner")
df3 = df3.drop(["starname"], axis=1)

Final_path = "./Results/Final_Results"
if not os.path.exists(Final_path):
    os.makedirs(Final_path)
df3.to_csv(Final_path + "/" + "res" + resonumber + "_RefEWPar.csv", index=False) #removed sep = ","
print ("done")